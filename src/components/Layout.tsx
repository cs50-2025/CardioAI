  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground transition-colors duration-300">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-card border-r border-border p-6">
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all group ${
                location.pathname === item.path 
                  ? 'bg-neon-blue text-primary-foreground font-bold shadow-[0_0_20px_rgba(0,242,255,0.2)]' 
                  : 'text-muted-foreground hover:text-foreground hover:bg-muted/30'
              }`}
            >
              <item.icon className={`w-5 h-5 ${location.pathname === item.path ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-foreground'}`} />
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="mt-auto pt-6 border-t border-border">
          <div className="flex items-center gap-3 px-4 mb-6">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-neon-blue to-neon-purple flex items-center justify-center font-bold text-primary-foreground">
              {user?.name?.[0]}
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {/* Header */}
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-border bg-background/80 backdrop-blur-md z-20">
          ...
          <div className="flex items-center gap-4">
            {user?.settings?.notifications && (
              <button className="p-2.5 bg-muted/30 border border-border rounded-xl text-muted-foreground hover:text-foreground hover:border-border transition-all relative">
                <Bell className="w-5 h-5" />
                <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border-2 border-[#050505]"></span>
              </button>
            )}
            <div className="h-8 w-[1px] bg-muted/50 mx-2 hidden lg:block"></div>
            <div className="hidden lg:flex items-center gap-3">
              <p className="text-sm font-medium text-muted-foreground">{new Date().toLocaleDateString()}</p>
            </div>
          </div>
        </header>
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.aside
              ...
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
              className="fixed inset-y-0 left-0 w-72 bg-card border-r border-border z-50 p-6 lg:hidden"
            >
                ...
                {navItems.map((item) => (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                      location.pathname === item.path ? 'bg-neon-blue text-primary-foreground font-bold' : 'text-muted-foreground'
                    }`}
                  >
